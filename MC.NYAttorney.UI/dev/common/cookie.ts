...
export class Cookie {
     ...
     */
    public static setCookieObject(name: string, value: Object) {

        let myWindow: any = window;
        let cookieStr = myWindow.escape(name) + '=' + myWindow.escape(JSON.stringify(value)) + ';';
     ...
     * @returns The Cookie's value
     */
    public static getCookieObject(name: string) {

        let myWindow: any = window;
        name = myWindow.escape(name);
        let regexp = new RegExp('(?:^' + name + '|;\\s*' + name + ')=(.*?)(?:;|$)', 'g');
        let result = regexp.exec(document.cookie);
        let ret = (result === null) ? null : myWindow.unescape(result[1]);
